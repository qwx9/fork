...
static u8int timerun, timerspeed;
static int timer;
...
{
	switch(a){
	case VSYNC:	// FIXME: there's your problem...?
		//fprint(2, "vsync %d,%d ← %d\n", ppux, ppuy, v);
		if((v & 2) != 0)
			flush();
		return;
	case VBLANK:
		//fprint(2, "vblank %d,%d ← %d\n", ppux, ppuy, v);
		if((v & 2) == 0){
			//flushscreen();
			ppuy = 0;
		}
...
iowrite(u8int a, u8int v)
{
	switch(a){
	case 4:
		fprint(2, "iowrite: edge detect %x %d\n", a, v);
	case 16+4:
		timerpre = 1;
		goto timer;
	case 5:
...
io(void)
{
	static double snddiv;

	timerstep();
	tiastep();
	tiastep();
	tiastep();
	if((snddiv += 1.0) >= SAMPDIV){
		snddiv -= SAMPDIV;
		sample();
	}
}
