...
arr(u8int d)	/* unofficial */
{
	...
	rP &= ~(FLAGN | FLAGZ | FLAGV | FLAGC);
	switch(r & 3 << 6){
	case 0xC0: rP |= FLAGC; break;
	case 0x80: rP |= FLAGC | FLAGV; break;
	case 0x40: rP |= FLAGV; break;
	}
	r >>= 1;
	if(r & 0x80) rP |= FLAGN;
	rA = r;
...
step(void)
{
	...
	if(nrdy){
		if(trace)
			print("%.4x -- | %.2x %.2x %.2x | %.2x %.2x | %3d %3d\n", curpc, rA, rX, rY, rS, rP, ppux-3, ppuy);
		io();
		return;
	}
	curpc = pc;
	...
	case 0x4A:
		...
		memread(pc);
		return;
	case 0x4B: lsr(nz(rA &= imm())); return;	/* unofficial */
	...
	case 0x87: memwrite(fetch8(), rA & rX); return;	/* unofficial */
	...
	case 0x97: memwrite(azpX(rY), rA & rX); return;	/* unofficial */
