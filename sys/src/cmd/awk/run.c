...
void setval(Cell *to, Cell *from)
{
	if (isstr(from)) {
		setsval(to, from->sval);
		if (isnum(from)) {
			to->fval = from->fval;
			to->tval |= NUM;
		}
		to->tval |= from->tval & (FCONV | TRAIL);
	} else if (isnum(from))
		setfval(to, from->fval);
	else
		funnyvar(from, "assign");
}
...
Cell *relop(Node **a, int n)	/* a[0 < a[1], etc. */
{
	...
	x = execute(a[0]);
	y = execute(a[1]);
	j = getfval(x) - getfval(y);
	if ((x->tval & (NUM|TRAIL)) == NUM && (y->tval & (NUM|TRAIL)) == NUM)
		i = j<0? -1: (j>0? 1: 0);
	else
...
Cell *cat(Node **a, int)	/* a[0] cat a[1] */
{
	...
	z = gettemp();
	z->sval = s;
	z->tval = STR | CON;	/* avoid implicit conversions to NUM */
	if (istemp(x))
		tfree(x);
	return(z);
}
