...
Awkfloat getfval(Cell *vp)	/* get float val of a Cell */
{
	...
	if (!isnum(vp)) {	/* not a number */
		if ((vp->tval & (CON|FCONV)) == 0) {
			if (to_number(vp->sval, &vp->fval, &tp)) {
...
static char *get_str_val(Cell *vp, char **fmt, uint gen)	/* get string val of a Cell */
{
	...
	if (isfld(vp) && donefld == 0)
		...
		recbld();
	if (isstr(vp) == 0
	|| vp->conv != 0 && isnum(vp) && !isfld(vp)
	&& (fmt == OFMT ^ (vp->tval & FMT) != 0 || vp->conv != gen)) {
		if (freeable(vp))
			xfree(vp->sval);
...
{
	return get_str_val(vp, CONVFMT, cfgen);
}

char *getpssval(Cell *vp)	/* get string val of a Cell for print */
{
	return get_str_val(vp, OFMT, ofgen);
}
