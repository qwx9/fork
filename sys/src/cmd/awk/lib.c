...
int readrec(char **pbuf, int *pbufsize, Biobuf *inf)	/* read one record into buf */
{
	...
	if ((sep = **RS) == 0) {
		sep = '\n';
		while ((c=Bgetrune(inf)) == '\n' && c != (Rune)Beof)	/* skip leading \n's */
			;
		if (c != (Rune)Beof)
			Bungetc(inf);
	}
	for (rr = buf; ; ) {
		for (; (c=Bgetrune(inf)) != sep && c != (Rune)Beof; ) {
			if (rr-buf+UTFmax > bufsize)
				if (!adjbuf(&buf, &bufsize, UTFmax+rr-buf, recsize, &rr, "readrec 1"))
					FATAL("input record `%.30s...' too long", buf);
			rr += runetochar(rr, &c);
		}
		if (**RS == sep || c == (Rune)Beof)
			break;
		if ((c = Bgetrune(inf)) == '\n' || c == (Rune)Beof) /* 2 in a row */
			break;
		if (!adjbuf(&buf, &bufsize, 1+UTFmax+rr-buf, recsize, &rr, "readrec 2"))
			FATAL("input record `%.30s...' too long", buf);
		*rr++ = '\n';
		rr += runetochar(rr, &c);
	}
	if (!adjbuf(&buf, &bufsize, 1+rr-buf, recsize, &rr, "readrec 3"))
		FATAL("input record `%.30s...' too long", buf);
	*rr = 0;
	   dprint( ("readrec saw <%s>, returns %d\n", buf, c == (Rune)Beof && rr == buf ? 0 : 1) );
	*pbuf = buf;
	*pbufsize = bufsize;
	return c == (Rune)Beof && rr == buf ? 0 : 1;
}
