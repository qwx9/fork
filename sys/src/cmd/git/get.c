...
fetchpack(Conn *c)
{
	...
	while(1){
		...
		if(interactive && spin++ % (Spintime * nelem(spinner)) == 0){
			i = spin / (Spintime * nelem(spinner)) % nelem(spinner);
			j = i + 1 == nelem(spinner) ? 0 : i + 1;
			fprint(2, "\b\b%C%C", spinner[i], spinner[j]);
		}
		packsz += n;
	}
	if(interactive && spin > 0)
		fprint(2, "\b\b");
	fprint(2, "\n");
	closeconn(c);
	if(seek(pfd, 0, 0) == -1)
